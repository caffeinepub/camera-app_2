...
export default function PhotoLightbox({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex flex-col bg-black/96 animate-fade-in"
      role="dialog"
      aria-modal="true"
    ...
    >
      ...
      <div className="flex-1 relative flex items-center justify-center overflow-hidden min-h-0">
        <img
          key={currentPhoto.id}
          src={imageUrl}
          alt={`Photo taken on ${formatTimestamp(currentPhoto.timestamp)}`}
...
function ThumbnailButton({ photo, index, isActive, currentIndex, onNext, onPrev }: ThumbnailButtonProps) {
  const handleClick = () => {
