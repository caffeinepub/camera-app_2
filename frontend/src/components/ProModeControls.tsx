...
import { Sun, Focus, Thermometer, Aperture } from 'lucide-react';
...
export default function ProModeControls({ videoTrack }: ProModeControlsProps) {
  ...
  const applyISO = (idx: number) => {
    ...
    try {
      ...
      videoTrack.applyConstraints({ advanced: [{ iso } as any] }).catch(() => {});
    } catch { /* not supported */ }
  };
  ...
  const applyShutter = (idx: number) => {
    ...
    if (label.includes('/')) {
      ...
      secs = 1 / parseFloat(parts[1]);
    } else {
      secs = parseFloat(label.replace('"', ''));
    }
    const exposureTime = secs * 1_000_000; // microseconds
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      videoTrack.applyConstraints({ advanced: [{ exposureTime, exposureMode: 'manual' } as any] }).catch(() => {});
    } catch { /* not supported */ }
  };
  ...
  const applyFocusMode = (mode: 'AF' | 'MF') => {
    ...
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      videoTrack.applyConstraints({ advanced: [{ focusMode: focusModeValue } as any] }).catch(() => {});
    } catch { /* not supported */ }
  };
  ...
  const applyFocusDistance = (val: number) => {
    ...
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      videoTrack.applyConstraints({ advanced: [{ focusDistance: val } as any] }).catch(() => {});
    } catch { /* not supported */ }
  };
  ...
  const applyWB = (preset: typeof WB_PRESETS[0]) => {
    ...
    try {
      if (preset.temp === 0) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        videoTrack.applyConstraints({ advanced: [{ whiteBalanceMode: 'continuous' } as any] }).catch(() => {});
      } else {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        videoTrack.applyConstraints({ advanced: [{ whiteBalanceMode: 'manual', colorTemperature: preset.temp } as any] }).catch(() => {});
      }
    } catch { /* not supported */ }
  };
    ...
    <div className="w-full px-2 py-2 space-y-3">
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5 text-amber">
            ...
            <span className="text-xs font-bold tracking-widest uppercase">ISO</span>
          </div>
          <span className="text-xs font-mono text-amber">{ISO_VALUES[isoIndex]}</span>
        </div>
        <Slider
          min={0}
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5 text-amber">
            ...
            <span className="text-xs font-bold tracking-widest uppercase">Shutter</span>
          </div>
          <span className="text-xs font-mono text-amber">{SHUTTER_VALUES[shutterIndex]}s</span>
        </div>
        <Slider
          min={0}
      ...
      <div className="space-y-1.5">
        <div className="flex items-center gap-1.5 text-amber">
          ...
          <span className="text-xs font-bold tracking-widest uppercase">Focus</span>
        </div>
        ...
        {focusMode === 'MF' && (
          <div className="space-y-1 pt-1">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-mono text-amber">{focusDistance.toFixed(2)}</span>
            </div>
            <Slider
              min={0}
              max={1}
      ...
      <div className="space-y-1.5">
        <div className="flex items-center gap-1.5 text-amber">
          ...
          <span className="text-xs font-bold tracking-widest uppercase">White Balance</span>
        </div>
