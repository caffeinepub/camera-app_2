...
export function useCinematicMode({ enabled, videoRef, blurMultiplier = 1.0 }: UseCinematicModeOptions) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (sharpCtx) {
        ...
        if (maskCtx) {
          const grad = maskCtx.createRadialGradient(cx, cy, Math.min(rx, ry) * 0.4, cx, cy, Math.max(rx, ry) * 1.15);
