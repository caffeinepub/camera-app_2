import type { BlurStrengthLevel } from "@/hooks/useBlurStrength";
...
export default function BlurStrengthToggle({
  level,
  onSelect,
}: BlurStrengthToggleProps) {
  return (
    <div className="flex items-center gap-1 glass-panel rounded-full px-1 py-1 border border-amber-500/20">
      <span className="text-amber-400/60 text-xs font-mono px-1.5 tracking-wide">
        BLUR
      </span>
      {LEVELS.map((l) => (
        <button
