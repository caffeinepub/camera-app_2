...
import type { CineColorProfile } from "@/hooks/useCineColorProfile";
...
export default function CinemaControlsPanel({
  ...
  const { isActive: anamorphicActive, toggle: toggleAnamorphic } =
    useAnamorphicOverlay();
    ...
    <div className="glass-panel border border-amber-500/20 rounded-2xl p-4 mx-2 mb-2 flex flex-col gap-4">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex flex-col items-center gap-2 flex-shrink-0">
          <span className="text-amber-400/60 text-xs font-mono tracking-wide">
            EXPOSURE
          </span>
          <div
            ref={wheelRef}
              ...
              {ticks.map((tick) => {
                ...
                return (
                  <line
                    ...
                    stroke={
                      isMajor ? "rgba(245,158,11,0.6)" : "rgba(245,158,11,0.25)"
                    }
        ...
        <div className="flex flex-col gap-3 flex-1">
          ...
          >
            <div
              className={`w-3 h-3 border-2 rounded-sm flex-shrink-0 ${anamorphicActive ? "border-amber-400" : "border-amber-400/30"}`}
            >
              {anamorphicActive && (
                <div className="w-full h-full bg-amber-400 scale-50" />
              )}
            </div>
            <span>2.39:1 Anamorphic</span>
            {anamorphicActive && (
              <span className="ml-auto text-amber-500 text-[9px] font-bold tracking-widest">
                ON
              </span>
            )}
          </button>
