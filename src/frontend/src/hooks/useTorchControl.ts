...
import type { TorchMode } from "./useCameraSettings";
...
export function useTorchControl(
  initialMode: TorchMode = "off",
): UseTorchControlReturn {
  ...
  const setTorchMode = useCallback(
    (mode: TorchMode, videoTrack: MediaStreamTrack | null) => {
