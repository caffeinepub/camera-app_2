...
export default function GalleryPage() {
  ...
  const _deletePhotoMutation = useDeletePhoto();
  ...
  const handlePhotoPrev = () => {
    ...
    setLightboxPhotoIndex(
      (lightboxPhotoIndex - 1 + photos.length) % photos.length,
    );
  };
  const handlePhotoDeleteFromLightbox = () => {
    ...
    if (photos.length <= 1) {
      ...
      const newIndex =
        lightboxPhotoIndex >= photos.length - 1
    ...
    <div className="min-h-full bg-black pb-4">
      ...
      {lightboxVideo && (
        <VideoLightbox
          ...
          onPrev={() => {
            if (lightboxVideoIndex > 0)
              setLightboxVideo(videos[lightboxVideoIndex - 1]);
          }}
          onNext={() => {
            if (lightboxVideoIndex < videos.length - 1)
              setLightboxVideo(videos[lightboxVideoIndex + 1]);
          }}
        />
      )}
